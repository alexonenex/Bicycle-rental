      ...
      <div class="col-12">
        <form>
          ...
          <div class="input-group verify">
            ...
            <button class="btn btn-outline-secondary" type="button" id="button-addon2" @click="obtain">获取验证码</button>
          </div>
  ...
  methods: {
    ...
    obtain() {
      if(isEmail(this.user.email)) {
        axios.get("http://localhost:8080/user/sendEmail/" + this.user.email)
        .then((response)=>{
