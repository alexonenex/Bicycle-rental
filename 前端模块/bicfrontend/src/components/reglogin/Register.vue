      ...
      <div class="col-12">
        <form>
          ...
          <div class="input-group verify">
            ...
            <button class="btn btn-outline-secondary" type="button" @click="obtain">获取验证码</button>
          </div>
...
export default {
  ...
  methods: {
    // 获取邮箱验证码
    obtain() {
      axios.get("http://localhost:8080/user/sendEmail/" + this.raDto.cEmail)
      .then((res) => {
        console.log(res);
    ...
    register() {
      ...
      // 进行注册操作
      axios.post('http://localhost:8080/user/registerAccount', this.raDto)
      .then((res) => {
        console.log(res);
        this.code = res.data.code;
      })
      if(this.code == 0) return alert("注册失败！");
